...
def coefficient(longSlipValue, latSlipValue, extremumValueLong, asymptoteValueLong, extremumValueLat, asymptoteValueLat, extremumSlipLong, asymptoteSlipLong, extremumSlipLat, asymptoteSlipLat):
    ...
    asymptoteValue = asymptoteValueLat + ((asymptoteValueLong-asymptoteValueLat) / asymptoteSlipLong ) * min(longSlipValue, asymptoteSlipLong)
    extremumValue = extremumValueLat + ((extremumValueLong-extremumValueLat) / extremumSlipLong ) * min(longSlipValue, extremumSlipLong)
...
for x in range(len(X)):
    for y in range(len(Y)):
        xs[x*len(Y)+y] = X[x]
        ys[x*len(Y)+y] = Y[y]
        value = coefficient(X[x], Y[y], 2*1.0, 2*0.75, 1.0, 0.75, 0.2, 0.4, 20, 40) 
        zs[x*len(Y)+y] = value
